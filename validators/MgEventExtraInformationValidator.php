...
class MgEventExtraInformationValidator
{
    ...
    public function __construct($options = array(), $messages = array())
    {
        ...
        $this->options  = array_merge(array('required' => false), $this->options);
    ...
    protected function configure()
    {
        ...
        $this->addMessage('url', '"%value%" is not a valid url.');

        $this->addOption('url');

        $this->addMessage('email', '"%value%" is not a valid email.');
    ...
    public function validateUrl($values, $field, $constraint)
    {
        if ($constraint) {
            if (isset($values[$field]) && $values[$field]) {
                $clean = (string) trim($values[$field]);
                ...
                if (!filter_var($clean, FILTER_VALIDATE_URL)) {
                    $this->errorSchema[$field][] = strtr($this->messages['url'], array('%value%' => $values[$field]));
                }
            }
        }
    }

    public function validateEmail($values, $field, $constraint)
    {
        if ($constraint) {
            if (isset($values[$field]) && $values[$field]) {
                $clean = (string) trim($values[$field]);
                ...
                if (!filter_var($clean, FILTER_VALIDATE_EMAIL)) {
                    $this->errorSchema[$field][] = strtr($this->messages['email'], array('%value%' => $values[$field]));
                }
            }
        }
    }
